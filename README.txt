...
Whenever a SYN flag was found on a packet, this meant a new flow has been found. I would then initialize
...
Then, to find the throughput for each flow, first recording the first timestamp in the flow. Then, for every packet that was
...
Then, to find the congestion windows, I took the initial RTT to be the time between the first SYN flag packet, and the response to this
...
Finally, to count the number of retransmissions, I kept track of when the receiver would send an ACK for a packet 
that equaled the ACK of the last packet it sent, meaning a duplicate ACK is occuring. For each consecutive dup ack, 
I would increment a counter, and I would
